...
async function main() {
  ...
  const createRes = await fetch(`${SUPABASE_URL}/storage/v1/bucket`, {
    ...
    body: JSON.stringify({ name: BUCKET_NAME, public: true, fileSizeLimit: 5 * 1024 * 1024, allowedMimeTypes: ['image/png', 'image/jpeg', 'image/webp'] })
  });
  ...
  console.log(`Bucket "${BUCKET_NAME}" criado com sucesso (p\u00fablico, max 5MB, png/jpeg/webp).`);
}
