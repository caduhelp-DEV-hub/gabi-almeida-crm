...
const BUCKETS = ['signatures', 'avatars', 'patient-photos'];
...
async function main() {
  ...
  for (const bucketName of BUCKETS) {
    ...
    if (exists) {
      ...
      if (!exists.public) {
        console.log(`Atualizando "${bucketName}" para público...`);
        ...
        if (!updateRes.ok) {
          console.error(`Erro ao atualizar bucket "${bucketName}":`, updateRes.status, await updateRes.text());
          process.exit(1);
        }
        console.log(`Bucket "${bucketName}" atualizado para público.`);
      }
    } else {
      ...
      const createRes = await fetch(`${SUPABASE_URL}/storage/v1/bucket`, {
        ...
        body: JSON.stringify({ name: bucketName, public: true, file_size_limit: 52428800 })
      });

      if (!createRes.ok) {
        console.error(`Erro ao criar bucket "${bucketName}":`, createRes.status, await createRes.text());
        process.exit(1);
      }
